...
class ParagraphSelector():
    ...
    def __init__(self,
        ...
        class ParagraphSelectorNet(BertPreTrainedModel):
            ...
            def forward(self, token_ids):
                ...
                # Alternatively outputs[0] is BERT's last layer's output
                ...
                with torch.no_grad(): #TODO de-activate this?
                    embedding = self.bert(token_ids)[-2][-1][:, 0, :]
                '''
                # The above command without "torch.no_grad()" fails for batch_size = 32 
                # because GPU memory is not sufficient; One should choose a smaller batch size (12) otherwise

                # if we choose torch.no_grad(), we are keeping the BERT model as it is, the only parameters 
                # that will be trained are self.linear() [quicker training]
                # However, it is advisable to let the BERT layer also trained(slow learning rate 1e-5 is for BERT)
    ...
    def train(self, train_data, epochs=10, batch_size=1, learning_rate=0.0001):
        ...
        optimizer = torch.optim.Adam(self.net.parameters(), lr=learning_rate) #learning_rate = 1e-5
        ...
        for epoch in range(epochs):
            ...
            for step, batch in enumerate(tqdm(train_data, desc="Iteration")):
                ...
                ''' To check if the input makes sense ''' 
                #print(self.tokenizer.convert_ids_to_tokens([ i.item() for i in inputs[0]]))
                #input("inputs")
    ...
    def evaluate(self, data, threshold=0.1, pad_token_id=0, text_length=512, try_gpu=True):
        ...
        #self.net.eval() # BERT model should be modified; eval() entails that we are probing for some task
                         # probing: if BERT embeddings are so informative, we can just take them and look
                         # if we can classify for what we want
    ...
    def save(self, savepath):
        ...
        self.net.save_pretrained(directory_name)
        self.tokenizer.save_pretrained(directory_name)
